    ...
    try {
      ...
      const recentGames = await Promise.all(games.slice(0, 10).map(async game => {
        ...
        if (game.startedAt && game.completedAt) {
          ...
          const durationMs = new Date(game.completedAt).getTime() - new Date(game.startedAt).getTime();
          ...
          durationText = estimatedMinutes === 1 ? '1 min' : `${estimatedMinutes} mins`;
        }
  ...
  app.get("/api/admin/transactions", requireAuth, async (req, res) => {
    try {
      ...
      if (month && year) {
        ...
        // Single date query (existing functionality) - default to today in IST
